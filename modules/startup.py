...
import threading
import time
from modules import redis_sub as redis
...
def startup(phenny, input):
   ...
   if hasattr(phenny.config, 'password'): 
      phenny.msg('NickServ', 'IDENTIFY %s' % phenny.config.nickserv_pass)
      time.sleep(5)
   ...
   # begin redis listener for messages when all channels joined
   redis.start(phenny)
